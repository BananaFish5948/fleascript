...
- [ ] テキストエリアに入力 → ボタンクリック → 生成テキスト表示の基本フローが動く
- [ ] 500文字超入力時にカウンターが赤くなりボタンが disabled になる
- [ ] 「ワンクリックでコピー」ボタンでクリップボードにコピーされる
- [ ] 👍クリック → feedback=positive が /api/feedback に送信される
- [ ] 👎クリック → 理由選択モーダルが表示され、選択後 feedback=negative + reason が送信される
- [ ] フッターの匿名意見箱が表示されている
- [ ] レスポンシブ対応（スマホ・PC両方で崩れない）
- [ ] バイラルクレジットがテキスト末尾に必ず含まれている（バックエンド側で付与済みのため表示するのみ）
